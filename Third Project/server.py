...
available_ips = [available_ip for available_ip in available_ips if available_ip not in reserved_ips.values() and available_ip != SERVER_IP]
...
while True:
    ...
    if data.options.by_code(53).data == b'\x01':
        if data.chaddr not in blocked_clients:
            requesting_ip = ''
            if data.chaddr in reserved_ips:
                requesting_ip = reserved_ips[data.chaddr]
            elif available_ips:
